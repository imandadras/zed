...
def chip_acknowledge(ack_value, readPort, verbose=False):
	...
	if not command_acknowledged:
		print("ERROR: command was not acknowledged", ack_value)
...
def set_fll(readPort, writePort, frequency, acknowledge=True, verbose=False):
	#setting the clock requires 24 instructions, so we need to acknowledge all 26 of them.
	instruction = (instructions.chip_setfll << 29) | (frequency)
	writePort.sendInt(instruction)

	if acknowledge:
		ack = chip_acknowledge(instructions.chip_setfll, readPort, verbose)
		if (not ack) and verbose:
			print("fll frequency set was not acknowledged.")
	return 1
...
def run_chip(readPort, writePort, reset, prog, mem_en, cycles_per_loop, loop_iterations=0, acknowledge=True, verbose=False):
	...
	# Start the chip
	instruction=(instructions.chip_start << 29) | (prog << 28) | (mem_en << 27) | (reset << 26) | (loop_iterations << 16) | cycles_per_loop
	writePort.sendInt(instruction)
...
def trigger_boot(readPort, writePort, acknowledge=True, verbose=False):
	instruction = (instructions.chip_boot << 29)
	writePort.sendInt(instruction)
	if acknowledge:
		ack = chip_acknowledge(instructions.chip_boot, readPort, verbose)
		if (not ack) and verbose:
			print("fll frequency set was not acknowledged.")
	return 1

def vddeon_ack(readPort, writePort, acknowledge=True, verbose=False):
	instruction = (instructions.chip_vddeon_ack << 29)
	writePort.sendInt(instruction)
	if acknowledge:
		ack = chip_acknowledge(instructions.chip_vddeon_ack, readPort, verbose)
		if (not ack) and verbose:
			print("fll frequency set was not acknowledged.")
	return 1
