...
for Vcs in range(67,81):
    ...
    for unit_time in range (0,10):
        ...
        for activation in range(-62,64):
            with open ('{}/{}/{}.csv'.format(Vcs/100, activation, unit_time),'r') as file:
        ...
        #m = (yp - ym)/ (xp - xm)
        #c = ((xp * ym)-(xm*yp))/ (xp - xm)
        last_point = next((x[0] for x in enumerate(UTs) if x[1]>20), 124)
        first_point = next((x[0] for x in enumerate(UTs) if x[1]>-20), 0)
    ...
    plt.xlabel("Output")
    plt.ylabel("Error")
    plt.title("Error according to the output")
    plt.legend()
    plt.grid(True)
    plt.show()
